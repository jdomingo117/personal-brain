...
- `src/`
  ...
  - `contexts/` - React Contexts.
    - `DataContext.tsx` - Global state management, handles authentication state and data fetching from Supabase. `refreshData()` sets `loading=true`, which unmounts the entire routed view (`Guards.tsx`'s `RequireOnboarded` returns a blank placeholder while `loading`) — any modal must defer `refreshData()` to its own close, not call it mid-flow, or it vanishes out from under the user. Also fires a background `sync-provider` call (deduped per session) for any connection whose `balance_as_of` is >30 min stale.
...
- `functions/` - Deno-based Edge Functions.
  ...
  - `upsert-account/` / `upsert-transactions/` / `upsert-profile/` / `delete-upload-batch/` - DB write operations. `upsert-transactions` computes dedupe hashes **and the `transfer_candidate` flag** server-side and returns `{inserted, skipped, needsReview}`.
  ...
  - `_shared/runLinkTransfers.ts` - The transfer-linking core, factored out of `link-transfers/index.ts` so `sync-provider` can call it in-process (no HTTP hop) right after committing a batch of provider rows.
...
### Tests
`npm test` (vitest, in `app/`) - 140 unit/corpus/pipeline tests over the real files in `Sample datasets/`, including the transfer-matcher corpus tests. The `e2e.test.ts` suite additionally drives the live stack and is skipped unless `SUPABASE_ANON_KEY` is set.
