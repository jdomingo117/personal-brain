-- The local safe-update extension rejects an unqualified DELETE even inside a
-- definer function. Scope the rebuild to accounts that actually have an
-- investment holding; this also preserves any future non-investment snapshot
-- rows if the valuation table is extended.
...
BEGIN
  ...
  WITH relevant_dates AS (
    SELECT DISTINCT h.account_id, p.price_date AS valuation_date
     ...
     WHERE EXISTS (SELECT 1 FROM public.investment_activities a
  ...
  UPDATE public.accounts a SET balance = latest.value_cents,
      ...
      updated_at = now()
    FROM (SELECT DISTINCT ON (account_id) account_id, value_cents, price_date
            FROM public.investment_account_valuations ORDER BY account_id, valuation_date DESC) latest
